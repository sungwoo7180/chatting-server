...
@Configuration // 이 클래스가 하나 이상의 @Bean 메서드를 선언하고 스프링 컨테이너에서 Bean 정의를 생성하고 런타임 시 해당 Bean 을 요청할 때 반환할 것을 나타냅니다.
public class WebConfig implements WebMvcConfigurer { // WebMvcConfigurer 인터페이스를 구현하는 WebConfig 라는 이름의 설정 클래스를 정의

    @Bean // 이 메서드가 스프링 컨테이너에 의해 관리되는 Bean 을 생성함을 나타냅니다.
    public WebMvcConfigurer corsConfigurer() { // corsConfigurer 라는 이름의 Bean 을 정의하는 메서드입니다.
        return new WebMvcConfigurer() { // WebMvcConfigurer의 익명 클래스를 반환
            ...
            public void addCorsMappings(@Nullable CorsRegistry registry) { // addCorsMappings 메서드는 CORS 설정을 구성 registry 매개변수는 null을 허용할 수 있습니다.
                if (registry == null) {
                    return;
                }// registry 가 null 이 아닌 경우에만 실행됩니다.
